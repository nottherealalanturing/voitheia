...
public class Therapist {
	...
	public static boolean findAnyWords(String[] words, String[] wordList) {
		...
		for (int i = 0; i < words.length; i++) {
			int inList = Therapist.inList(words[i], wordList);
			if (inList != -1) {
				return true;
			}
		}
	...
	public static String initialProcessing(String userInput) {
		...
		char[] userInputArray = Therapist.filterChars(userInput);

		// call removeDuplicateSpaces
		Therapist.removeDuplicateSpaces(userInputArray);
		...
		String longest = Therapist.findLongest(sentences);
		return longest;
	}
	...
	public static String[] replacePairs(String[] words, String[] beforeList, 
		...
		for (int i = 0; i < words.length; i++) {
			int match = Therapist.inList(words[i], beforeList);
			if (match != -1) {
		...
		String strWords = Therapist.assemblePhrase(words);
	...
	public static String matchResponse(String[] words) {
		...
		if (words[0] == null) {
			...
			= Therapist.chooseString(Config.NO_MATCH_RESPONSES);
		...
		for (int i = 0; i < Config.RESPONSE_TABLE.length; i++) {
			...
			String[] pattern = 
					Therapist.findPatternInSentence(Config.RESPONSE_TABLE[i][0], 
							words);

			// choose a draft response from the response set (chooseString)
			if (pattern != null) {
				String draftResponse = 
						Therapist.chooseString(Config.RESPONSE_TABLE[i][1]);

				// Prepare the response by replacing <1>, <2>, etc with user
				// phrases found by matching the input to the pattern
				// (prepareResponse)
				String response = Therapist.prepareResponse(draftResponse, pattern);
				return response;
			}
	 ...
	 * program terminates.
	 */
	public static String interactive(String ogText) {
		
		String dialogue = "";              
                dialogue = processInput(ogText);
                return dialogue;
